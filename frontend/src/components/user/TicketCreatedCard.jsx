...
export default function TicketCreatedCard({ ticket }) {
  ...
  return (
    <div className="msg-in flex justify-start">
      <div className="max-w-[85%] bg-white border border-sber-100 rounded-2xl p-4 text-sm space-y-2.5 shadow-soft">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-sber-500" />
          <span className="font-medium text-ink-900">Заявка №{ticket.id} создана</span>
        </div>
        ...
        <p className="text-xs text-ink-700 bg-ink-50 rounded-xl px-3 py-2.5 leading-relaxed">
          «{ticket.original}»
        </p>
