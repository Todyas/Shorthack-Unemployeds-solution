  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
      <div className="lg:col-span-5 surface p-5">
        <h3 className="font-display font-semibold mb-3 text-sm">Текст обращения</h3>
        <p className="text-sm text-ink-700 leading-relaxed whitespace-pre-line border-l-2 border-ink-200 pl-3.5">{ticket.original}</p>
            ...
            <div className="space-y-1.5">
              {siblings.map((s) => (
                <button
                  ...
                  className="w-full text-left text-xs px-2.5 py-1.5 rounded-xl bg-ink-50 hover:bg-ink-100 disabled:cursor-default disabled:hover:bg-ink-50"
                >
                  <span className="text-ink-400">#{s.id}</span> {s.title}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="lg:col-span-7 surface-ai p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="text-xs text-ai-600 font-medium flex items-center gap-1.5 hover:text-ai-700 disabled:opacity-60"
          >
        ...
        <div className="space-y-4 text-sm">
          <div>
            <p className="eyebrow mb-1">Краткая суть</p>
            <p className="text-ink-900">{ticket.summary}</p>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="eyebrow mb-1">Категория</p>
              <p className="font-medium text-ink-900">{ticket.category}</p>
            </div>
            <div>
              <p className="eyebrow mb-1">Приоритет</p>
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${priorityStyle[ticket.priority]}`}>
                {priorityLabel[ticket.priority]}
              </span>
            </div>
            <div>
              <p className="eyebrow mb-1">Решение ИИ</p>
              <p className="font-medium text-ink-900">{ticket.actionTypeLabel}</p>
            </div>
            <div>
              <p className="eyebrow mb-1">Шаблон базы знаний</p>
              <p className="font-medium text-ink-900">{ticket.kbTemplateId || 'не найден'}</p>
            </div>
          </div>

          <div>
            <p className="eyebrow mb-1">Обоснование ИИ</p>
            <p className="text-ink-700">{ticket.reasoning || 'ИИ не указал обоснование для этой заявки.'}</p>
          </div>

          <div>
            <p className="eyebrow mb-1">Недостающая информация</p>
